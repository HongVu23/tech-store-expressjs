...
class SmartPhoneVariantMiddleware extends MainProductVariantMiddleware {
    ...
    checkAllRequiredFields(fields, actionType) {
        ...
        let { ram, rom, color, price, discount, quantity } = fields

        if (actionType === 'update') {

            if (ram || rom || color || quantity) {
                return { isValid: false, error: { message: 'Updating ram, rom, color or quantity field is not allowed' } }
            }
        ...
        if (actionType === 'create') {
            ...
            if (!ram || !rom || !color || !price || !discount || !quantity) {
                return { isValid: false, error: { message: 'All fields are required' } }
            }
    ...
    async checkDuplicateProduct(fields, modelName, variantModel) {
        ...
        const { id, ram, rom, color } = fields

        let duplicate

        try {
            duplicate = await variantModel.findOne({ [modelName]: id, ram, rom, color }).lean().exec()
        } catch (error) {
    ...
    fileFilterCreator(actionType, modelName, model, variantModel) {
        ...
        const fileFilter = async (req, file, cb) => {
            ...
            const { ram, rom, color, price, discount, quantity } = req.body
            ...
            const checkAllRequiredFieldsResult = checkAllRequiredFields({ ram, rom, color, price, discount, quantity }, actionType)
            ...
            if (actionType === 'create') {
                const checkDuplicateProductResult = await checkDuplicateProduct({ id, ram, rom, color }, modelName, variantModel)
    ...
    async checkDuplicateCNPV(req, res, next) {
        ...
        const { ram, rom, color } = req.body

        const duplicate = await this.variantModel.findOne({ [this.name]: id, ram, rom, color }).lean().exec()
...
module.exports = SmartPhoneVariantMiddleware